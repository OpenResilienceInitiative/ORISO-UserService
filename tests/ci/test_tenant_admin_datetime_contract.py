...
TENANT_SPECS = {
    "tenantservice": ROOT / "services" / "tenantservice.yaml",
    "tenantadminservice": ROOT / "services" / "tenantadminservice.yaml",
}
...
class TenantAdminDateTimeContractTest(unittest.TestCase):
    def test_local_timestamp_fields_remain_plain_wire_strings_in_all_tenant_clients(self):
        for client_name, spec_path in TENANT_SPECS.items():
            ...
            for property_name in LOCAL_TIMESTAMP_PROPERTIES:
                with self.subTest(client=client_name, property=property_name):
