...
- Admin and consultant profile mutations use the focused
  `IdentityProfileUpdater` port and provider-neutral `IdentityProfileUpdate`
  value. Each mutation resolves and reads the target identity once. An
  unchanged email performs no availability search and one update; a changed
  email performs one availability search and at most one update. Web and
  Keycloak DTOs stay outside the port, and the broad command client no longer
  exposes profile mutation.
- Legacy Rocket.Chat group operations, their session-assignment and
  agency-relation callers, and `SessionSupervisorFacade` no longer carry an
  unused `IdentityClient` constructor dependency. This removes dead
  broad-client wiring without changing group-condition or supervision
  behavior.
...
- Current-account password and preferred-language changes now consume the
  ...
  language stops after the read.
- Admin, consultant and user provisioning, both asker-import paths and
  self-service password reset now consume the focused
  `IdentityPasswordUpdater` port. Every password attempt resolves the target
  identity once and performs at most one provider reset. Provider credential
  construction and password-policy error translation remain inside the
...
- Deletion workflows, admin deletion and provisioning rollback now consume the
  focused `IdentityAccountRemover` port. A normal removal resolves the target
  once and performs one remove call. An unauthorized removal refreshes the
  admin session once and retries the complete lookup/remove operation at most
  once. Provider not-found handling remains idempotent, rollback remains
  best-effort, and application deletion/rollback sequencing is unchanged. The
  broad command client no longer exposes deletion or rollback.
...
The remaining broad client debt is identity creation and dummy-email commands.
...
profile-write contract keeps both consumers on the focused provider-neutral
port and rejects profile mutation on the broad command client. Adapter
interaction tests enforce one target read, zero or one email-availability
search and at most one profile update. The
...
provider update. The account-removal contract requires strict deletion and
best-effort rollback consumers to use `IdentityAccountRemover`, rejects both
commands on the broad client and protects the shared Spring mocks. Adapter
interaction tests enforce one lookup/remove attempt normally and at most one
complete retry after one unauthorized-session refresh. The
