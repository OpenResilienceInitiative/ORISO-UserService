...
class DeleteInactiveSessionsAndUserSchedulerReplicaIT {
  ...
  @Test
  void twoSchedulerInstancesTriggerOneInactiveSessionDeletionWorkflow() throws Exception {
    var deletionService = mock(DeleteInactiveSessionsAndUserService.class);
    ...
    verify(deletionService, times(1)).deleteInactiveSessionsAndUsers();
  }

  private DeleteInactiveSessionsAndUserScheduler newScheduler(
      DeleteInactiveSessionsAndUserService deletionService,
      TenantContextProvider tenantContextProvider) {
    var scheduler =
        new DeleteInactiveSessionsAndUserScheduler(
            deletionService, tenantContextProvider, taskClaimService);
    ReflectionTestUtils.setField(scheduler, "sessionInactiveDeleteWorkflowEnabled", true);
    ReflectionTestUtils.setField(scheduler, "claimDuration", Duration.ofHours(12));
    return scheduler;
  }

  private void run(
      DeleteInactiveSessionsAndUserScheduler scheduler,
