...
class RedisOneTimeTokenStoreIT {
  ...
  @Test
  void redisKeysDoNotExposeBearerTokensOrSubjectIdentifiers() {
    Instant expiresAt = Instant.ofEpochMilli(Instant.now().plusSeconds(30).toEpochMilli());
    String bearerToken = "secret-bearer-token";
    String subjectId = "sensitive-account-id";

    firstInstance.store("reset", bearerToken, subjectId, expiresAt, true);

    assertThat(redisTemplate.keys(prefix + "*"))
        .allSatisfy(
            key -> {
              assertThat(key).doesNotContain(bearerToken);
              assertThat(key).doesNotContain(subjectId);
            });
  }

  @Test
  void claimCanBeRestoredOnlyForItsRemainingTtl() {
    Instant expiresAt = Instant.ofEpochMilli(Instant.now().plusMillis(800).toEpochMilli());
