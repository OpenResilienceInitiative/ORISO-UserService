...
class SenderOrganisationTest {
  ...
  void overriddenBy_replacesOnlyTheFieldsTheOverrideHas() {
    SenderOrganisation base = new SenderOrganisation("Betreiber", "Betreiberweg 1", "info@b.example");

    SenderOrganisation merged = base.overriddenBy(new SenderOrganisation("Träger", null, null));

    assertThat(merged).isEqualTo(new SenderOrganisation("Träger", "Betreiberweg 1", "info@b.example"));
  }
}
